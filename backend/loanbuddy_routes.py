...
def handle_document():
    try:
        ...
        # Extract key information first for faster processing
        text = result["text"]
...
def extract_key_info(text):
    ...
    if re.search(r'loan amount.*?(\d[\d,]*)', text, re.I):
        key_info["loan_amount"] = re.search(r'loan amount.*?(\d[\d,]*)', text, re.I).group(1)
    if re.search(r'interest.*?(\d+\.?\d*)%', text, re.I):
        key_info["interest_rate"] = re.search(r'interest.*?(\d+\.?\d*)%', text, re.I).group(1)
    if re.search(r'tenure.*?(\d+)\s*(month|year)', text, re.I):
        key_info["tenure"] = re.search(r'tenure.*?(\d+)\s*(month|year)', text, re.I).group(1)
    if re.search(r'(personal|home|car|business)\s+loan', text, re.I):
        key_info["type"] = re.search(r'(personal|home|car|business)\s+loan', text, re.I).group(1)
