...
export default function FloatingAssistant() {
  ...
  const convertSpeechToText = async (audioBlob) => {
    ...
    try {
      ...
      const response = await fetch(process.env.NEXT_PUBLIC_SARVAM_STT_API_URL, {
        method: 'POST',
        headers: {
  ...
  useEffect(() => {
    if (isOpen && messages.length === 0) {
      ...
      setMessages([{
        ...
        content: greeting,
        suggestions: [
          t('common.loanAssistant'),
          t('common.eligibilityCheck'),
          t('common.myLoans')
        ]
      }]);
    ...
    if (assistantResponse.action === 'navigate' && assistantResponse.path) {
      ...
      if (currentLocation !== assistantResponse.path) {
        ...
        setTimeout(async () => {
          ...
          // Add contextual guidance after navigation
          if (assistantResponse.guidance) {
            const guidanceMessage = {
              type: 'assistant',
              content: assistantResponse.guidance,
              suggestions: assistantResponse.suggestions || []
            };
            setMessages(prev => [...prev, guidanceMessage]);
            await speakWithSarvam(assistantResponse.guidance);
          }
        }, 1500);
      }
    }
  ...
  const handleUserInput = async (text) => {
    ...
    // Add user message in the current language
    setMessages(prev => [...prev, { 
      type: 'user', 
      content: text 
    }]);
    ...
    try {
      ...
      if (data.success && data.data) {
        ...
        // Handle navigation and guidance
